...
## Invoice Upload & AI Parsing (Phase 1 — PFG)

### DB Schema
- [x] Add `invoices` table (id, vendor, invoiceNumber, invoiceDate, totalAmount, imageUrls JSON, status enum: pending/reviewed/applied, createdBy, createdAt, updatedAt)
- [x] Add `invoice_lines` table (id, invoiceId, itemId nullable, itemNumber, description, pack, size, shippedQty, unitPrice, extension, matchStatus enum: matched/unmatched/skipped)
- [x] Apply migration SQL to production TiDB

### Backend
- [x] Add `invoices.upload` procedure — accepts base64 image array, stores to S3, creates invoice record with status=pending
- [x] Add `invoices.parse` procedure — sends stored image URLs to LLM vision, extracts invoice header + line items, saves to invoice_lines
- [x] Add `invoices.list` procedure — returns all invoices with status and summary
- [x] Add `invoices.getWithLines` procedure — returns invoice + all lines with match status
- [x] Add `invoices.applyToInventory` procedure — for each matched line, adds shippedQty to item's running inventory balance
- [x] Add `invoices.updateLine` procedure — allows manual correction of itemId match or quantity before applying

### Frontend
- [x] Add "Invoices" nav item in AppLayout (admin-only)
- [x] Build /invoices page — list of all invoices with status badges (Pending / Reviewed / Applied)
- [x] Build invoice upload flow — camera/file picker, multi-image support, upload button
- [x] Build invoice review screen — show parsed lines, match status, allow manual correction of item matches
- [x] Build "Apply to Inventory" confirmation step
- [x] Dashboard: update Current Stock to factor in delivery receipts since last count

### Tests
- [x] Vitest tests for invoice line parsing logic
- [x] Vitest tests for inventory balance calculation (count + deliveries since count)
