...
function ExportButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick}
      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-muted text-muted-foreground hover:bg-muted/80 transition-colors shrink-0">
...
function getPresetRange(days: number): { start: string; end: string } {
  ...
  if (days === -1) { start.setMonth(0, 1); start.setHours(0, 0, 0, 0); }
  else { start.setDate(end.getDate() - days); }
  return { start: toIso(start), end: toIso(end) };
}

function DateRangePicker({ startDate, endDate, onStartChange, onEndChange, activePreset, onPreset }: {
  startDate: string; endDate: string;
  onStartChange: (v: string) => void; onEndChange: (v: string) => void;
  activePreset: number | null; onPreset: (days: number) => void;
}) {
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {PRESETS.map(p => (
          <button key={p.days} onClick={() => onPreset(p.days)}
            className={cn("px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors",
              activePreset === p.days ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80")}>
            {p.label}
          </button>
        ))}
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1 bg-muted/50 rounded-lg px-3 py-1.5 border border-border">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={cn("inline-flex px-2 py-0.5 rounded-full text-xs font-semibold capitalize", map[status] ?? map.pending)}>{status}</span>;
}
...
function CogsDrilldownView({ periodLabel, periodStart, periodEnd, onBack }: {
  ...
  const totalCost = useMemo(() => (data ?? []).reduce((s: number, r: DrillRow) => s + r.consumptionCost, 0), [data]);

  const handleExport = () => {
    if (!data) return;
    downloadCsv(`cogs-drilldown-${periodStart}.csv`,
      data.map(r => [r.itemName, r.vendor ?? "", r.category ?? "", r.openingQty.toFixed(2), r.receiptsQty.toFixed(2), r.closingQty.toFixed(2), r.consumptionQty.toFixed(2), r.consumptionCost.toFixed(2)]),
      ["Item", "Vendor", "Category", "Opening Qty", "Received Qty", "Closing Qty", "Consumed Qty", "Consumption Cost"]);
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft size={16} />Back to COGS overview
        </button>
      ...
      {isLoading ? <ReportSkeleton /> : !data || data.length === 0 ? (
        <EmptyState icon={<TrendingUp size={40} className="text-muted-foreground/40" />} title="No data for this period" description="No stock events found between these dates." />
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3">
            <SummaryCard label="Total COGS" value={fmtExact$(totalCost)} sub={`${data.length} items`} />
          </div>
          {Object.entries(byCategory).sort(([a], [b]) => a.localeCompare(b)).map(([cat, items]: [string, DrillRow[]]) => {
                      ...
                      <tbody>
                        {items.map((row: DrillRow) => (
                          <tr key={row.itemId} className="border-b border-border last:border-0 hover:bg-muted/20">
                            <td className="px-4 py-2.5"><div className="font-medium text-foreground text-sm">{row.itemName}</div>{row.vendor && <div className="text-xs text-muted-foreground">{row.vendor}</div>}</td>
...
function WeeklyCogsReport() {
  ...
  function handlePreset(days: number) {
    ...
    setStartDate(range.start); setEndDate(range.end); setActivePreset(days);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <ReportSkeleton /> : !data || data.length === 0 ? (
        <EmptyState icon={<TrendingUp size={40} className="text-muted-foreground/40" />} title="No COGS data yet" description="Apply invoices and complete inventory counts to see cost of goods data." />
      ) : (
        <>
...
function PriceChangesReport() {
  ...
  const { data, isLoading } = trpc.reports.priceChangesByRange.useQuery(
    { startDate, endDate }, { enabled: !!startDate && !!endDate }
  );
  ...
  const handleExport = () => {
    if (!data) return;
    downloadCsv(`price-changes-${startDate}-to-${endDate}.csv`,
      data.map(r => [r.itemName, r.vendor ?? "", r.oldPrice.toFixed(2), r.newPrice.toFixed(2), r.diff.toFixed(2), r.pctChange.toFixed(1) + "%", r.source ?? "", fmtDate(r.changedAt)]),
      ["Item", "Vendor", "Old Price", "New Price", "Change $", "Change %", "Source", "Date"]);
  };

  const increases = (data ?? []).filter(r => r.diff > 0);
  const decreases = (data ?? []).filter(r => r.diff < 0);
  const avgIncrease = increases.length > 0 ? increases.reduce((s, r) => s + r.pctChange, 0) / increases.length : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <ReportSkeleton /> : !data || data.length === 0 ? (
        <EmptyState icon={<Tag size={40} className="text-muted-foreground/40" />} title="No price changes in this range" description="Price changes are tracked automatically when you re-upload an order guide with updated prices." />
      ) : (
        <>
          <div className="flex items-start justify-between gap-4">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 flex-1">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.map(row => (
                    <tr key={row.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3"><div className="font-medium text-foreground">{row.itemName}</div>{row.vendor && <div className="text-xs text-muted-foreground">{row.vendor}</div>}</td>
                      ...
                      <td className="px-4 py-3 text-right"><span className={cn("font-semibold", row.diff > 0 ? "text-destructive" : "text-green-600")}>{row.diff > 0 ? "+" : ""}{fmtExact$(row.diff)} ({fmtPct(row.pctChange)})</span></td>
...
interface CountCompareRow {
  itemId: number; itemName: string; vendor: string | null; category: string | null;
  unit: string; parLevel: number | null; price: number | null;
  qtyA: number | null; qtyB: number | null;
  costA: number | null; costB: number | null;
  diff: number | null; costDiff: number | null;
}

function CompareView({ compareData, sessionA, sessionBId, allSessions }: {
  compareData: CountCompareRow[];
  sessionA: { id: number; name: string | null };
  sessionBId: number;
  allSessions: Array<{ sessionId: number; sessionName: string | null; completedAt: Date | null }>;
}) {
  const sessionB = allSessions.find(s => s.sessionId === sessionBId);
  const labelA = sessionA.name ?? `Count #${sessionA.id}`;
  const labelB = sessionB?.sessionName ?? `Count #${sessionBId}`;
  const byCategory = useMemo(() => {
    return compareData.reduce((acc: Record<string, CountCompareRow[]>, r: CountCompareRow) => {
      const cat = r.category ?? "Uncategorized";
      if (!acc[cat]) acc[cat] = [];
      acc[cat].push(r);
      return acc;
    }, {} as Record<string, CountCompareRow[]>);
  }, [compareData]);
  const totalCostA = compareData.reduce((s: number, r: CountCompareRow) => s + (r.costA ?? 0), 0);
  const totalCostB = compareData.reduce((s: number, r: CountCompareRow) => s + (r.costB ?? 0), 0);
  const totalDiff = totalCostB - totalCostA;
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        <SummaryCard label={labelA} value={fmtExact$(totalCostA)} sub="Total value" />
        <SummaryCard label={labelB} value={fmtExact$(totalCostB)} sub="Total value" />
        <SummaryCard label="Difference" value={fmtExact$(totalDiff)} sub={totalDiff >= 0 ? "Increase" : "Decrease"} valueClass={totalDiff >= 0 ? "text-destructive" : "text-green-600"} />
      </div>
      {(Object.entries(byCategory) as [string, CountCompareRow[]][]).sort(([a], [b]) => a.localeCompare(b)).map(([cat, items]) => (
        <div key={cat}>
          ...
          <div className="rounded-xl border border-border overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="bg-muted/30 border-b border-border">
                  <th className="text-left px-4 py-2 text-xs font-semibold text-muted-foreground">Item</th>
                  <th className="text-right px-4 py-2 text-xs font-semibold text-muted-foreground">{labelA}</th>
                  <th className="text-right px-4 py-2 text-xs font-semibold text-muted-foreground">{labelB}</th>
                  <th className="text-right px-4 py-2 text-xs font-semibold text-muted-foreground">Δ Qty</th>
                  <th className="text-right px-4 py-2 text-xs font-semibold text-muted-foreground">Δ Cost</th>
                </tr></thead>
                <tbody>
                  {items.map((row: CountCompareRow) => (
                    <tr key={row.itemId} className={cn("border-b border-border last:border-0 hover:bg-muted/20", row.diff !== null && row.diff < 0 && "bg-amber-50/50 dark:bg-amber-950/10")}>
                      <td className="px-4 py-2.5"><div className="font-medium text-foreground text-sm">{row.itemName}</div>{row.vendor && <div className="text-xs text-muted-foreground">{row.vendor}</div>}</td>
                      <td className="px-4 py-2.5 text-right text-muted-foreground">{row.qtyA?.toFixed(2) ?? "—"}</td>
                      <td className="px-4 py-2.5 text-right text-muted-foreground">{row.qtyB?.toFixed(2) ?? "—"}</td>
                      <td className="px-4 py-2.5 text-right">{row.diff !== null ? <span className={cn("font-medium text-sm", row.diff > 0 ? "text-green-600" : row.diff < 0 ? "text-destructive" : "text-muted-foreground")}>{row.diff > 0 ? "+" : ""}{row.diff.toFixed(2)}</span> : "—"}</td>
                      <td className="px-4 py-2.5 text-right">{row.costDiff !== null ? <span className={cn("font-semibold text-sm", row.costDiff > 0 ? "text-green-600" : row.costDiff < 0 ? "text-destructive" : "text-muted-foreground")}>{row.costDiff > 0 ? "+" : ""}{fmtExact$(row.costDiff)}</span> : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function CountSessionDetailView({ detail, compareSessionId, allSessions }: {
  detail: CountSessionDetailData;
  compareSessionId: number | null;
  allSessions: Array<{ sessionId: number; sessionName: string | null; completedAt: Date | null }>;
}) {
  const { data: compareDataRaw } = trpc.reports.compareCountSessions.useQuery(
    { sessionIdA: detail.session.id, sessionIdB: compareSessionId! },
    { enabled: compareSessionId !== null }
  );
  const compareData = compareDataRaw as CountCompareRow[] | undefined;
  const totalCost = detail.entries.reduce((s, e) => s + (e.totalCost ?? 0), 0);
  const belowPar = detail.entries.filter(e => e.belowPar).length;

  const handleExport = () => {
    if (compareData && compareData.length > 0 && compareSessionId !== null) {
      const compareSession = allSessions.find(s => s.sessionId === compareSessionId);
      downloadCsv(`count-compare-${detail.session.id}-vs-${compareSessionId}.csv`,
        (compareData as CountCompareRow[]).map(r => [r.itemName, r.vendor ?? "", r.category ?? "", r.qtyA?.toFixed(2) ?? "—", r.qtyB?.toFixed(2) ?? "—", r.diff?.toFixed(2) ?? "—", r.costA?.toFixed(2) ?? "—", r.costB?.toFixed(2) ?? "—", r.costDiff?.toFixed(2) ?? "—"]),
        ["Item", "Vendor", "Category", `Qty (${detail.session.name ?? `#${detail.session.id}`})`, `Qty (${compareSession?.sessionName ?? `#${compareSessionId}`})`, "Qty Diff", "Cost A", "Cost B", "Cost Diff"]);
    } else {
      downloadCsv(`count-session-${detail.session.id}.csv`,
        detail.entries.map(e => [e.itemName, e.vendor ?? "", e.category ?? "", e.quantity.toFixed(2), e.unit, e.parLevel?.toFixed(2) ?? "", e.price?.toFixed(2) ?? "", e.totalCost?.toFixed(2) ?? "", e.belowPar ? "Yes" : "No"]),
        ["Item", "Vendor", "Category", "Quantity", "Unit", "Par Level", "Price", "Total Cost", "Below Par"]);
    }
  };

  const byCategory = useMemo(() => {
    return detail.entries.reduce((acc: Record<string, CountEntry[]>, e) => {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <SummaryCard label="Total Value" value={fmtExact$(totalCost)} sub={`${detail.entries.length} items`} />
        <SummaryCard label="Below Par" value={String(belowPar)} sub="Items below par level" valueClass={belowPar > 0 ? "text-amber-600" : "text-green-600"} />
      </div>
      <ExportButton onClick={handleExport} />
      {compareData && compareData.length > 0 ? (
        <CompareView compareData={compareData as CountCompareRow[]} sessionA={detail.session} sessionBId={compareSessionId!} allSessions={allSessions} />
      ) : (
        Object.entries(byCategory).sort(([a], [b]) => a.localeCompare(b)).map(([cat, items]) => {
          const catTotal = items.reduce((s, e) => s + (e.totalCost ?? 0), 0);
          return (
            <div key={cat}>
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{cat}</h3>
              ...
              <div className="rounded-xl border border-border overflow-hidden">
                <table className="w-full text-sm">
                  <tbody>
                    {items.map(entry => (
                      <tr key={entry.itemId} className={cn("border-b border-border last:border-0", entry.belowPar && "bg-amber-50 dark:bg-amber-950/20")}>
                        <td className="px-4 py-2.5"><span className="font-medium text-foreground">{entry.itemName}</span>{entry.belowPar && <span className="ml-2 text-xs text-amber-600 font-medium">↓ below par</span>}</td>
                        <td className="px-4 py-2.5 text-right text-muted-foreground">{entry.quantity.toFixed(2)} {entry.unit}{entry.parLevel !== null && <span className="text-xs ml-1">/ {entry.parLevel} par</span>}</td>
                        <td className="px-4 py-2.5 text-right font-medium text-foreground">{entry.totalCost !== null ? fmtExact$(entry.totalCost) : "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function CountHistoryReport() {
  ...
  const { data: detail, isLoading: detailLoading } = trpc.reports.countSessionDetail.useQuery(
    { sessionId: selectedSession! }, { enabled: selectedSession !== null }
  );

  const handleExportList = () => {
    if (!data) return;
    downloadCsv("count-history.csv",
      data.map(r => [r.sessionName ?? `Count #${r.sessionId}`, fmtDate(r.completedAt ?? r.createdAt), String(r.entryCount), r.totalCost.toFixed(2), String(r.belowParCount)]),
      ["Session", "Date", "Items", "Total Value", "Below Par"]);
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <button onClick={() => { setSelectedSession(null); setCompareSessionId(null); }}
          ...
          {data && data.length > 1 && (
            <div className="flex items-center gap-2">
              ...
              <select value={compareSessionId ?? ""} onChange={e => setCompareSessionId(e.target.value ? Number(e.target.value) : null)}
                className="text-xs bg-muted border border-border rounded-lg px-2 py-1.5 text-foreground outline-none">
                <option value="">Compare with...</option>
                {data.filter(s => s.sessionId !== selectedSession).map(s => (
                  <option key={s.sessionId} value={s.sessionId}>
                    {s.sessionName ?? `Count #${s.sessionId}`} — {fmtDate(s.completedAt ?? s.createdAt)}
                  </option>
                ))}
              </select>
            </div>
        ...
        {detailLoading ? <ReportSkeleton /> : detail ? (
          <CountSessionDetailView detail={detail as CountSessionDetailData} compareSessionId={compareSessionId} allSessions={data ?? []} />
  ...
  if (isLoading) return <ReportSkeleton />;
  if (!data || data.length === 0) {
    return <EmptyState icon={<ClipboardList size={40} className="text-muted-foreground/40" />} title="No completed counts" description="Complete an inventory count to see history here." action={<Link href="/count"><Button size="sm">Start Count</Button></Link>} />;
  }
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <ExportButton onClick={handleExportList} />
      </div>
      <div className="rounded-xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map(row => (
                <tr key={row.sessionId} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors cursor-pointer"
                  onClick={() => setSelectedSession(row.sessionId)}>
                  <td className="px-4 py-3"><div className="font-medium text-foreground">{row.sessionName ?? `Count #${row.sessionId}`}</div><div className="text-xs text-muted-foreground">{fmtDate(row.completedAt ?? row.createdAt)}</div></td>
                  ...
                  <td className="px-4 py-3 text-right">{row.belowParCount > 0 ? <span className="text-amber-600 font-medium">{row.belowParCount}</span> : <span className="text-green-600">0</span>}</td>
                  <td className="px-4 py-3 text-right"><ChevronRight size={16} className="text-muted-foreground ml-auto" /></td>
                </tr>
              ))}
            </tbody>
          </table>
...
const TABS: { id: ReportTab; label: string; icon: React.ReactNode }[] = [
  { id: "cogs", label: "COGS", icon: <TrendingUp size={18} /> },
  { id: "invoices", label: "Invoices", icon: <FileText size={18} /> },
  { id: "prices", label: "Prices", icon: <Tag size={18} /> },
  { id: "counts", label: "Counts", icon: <ClipboardList size={18} /> },
];
      ...
      <div className="flex gap-1 bg-muted/50 rounded-xl p-1">
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={cn("flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all flex-1 justify-center",
